...
public class AutoSeriesController {
	...
	public @ResponseBody String crawlSeries(Integer seriesId) {
		AutoSeries series = autoSeriesService.findById(seriesId);
		if (null != series && null != series.getAutoHomeId() && 0 != series.getAutoHomeId()) {
			autoHomeCrawlService.crawlModel(seriesId, series.getAutoHomeId());
			return ResultWrapper.success("按车系同步数据成功");
		}
		return ResultWrapper.fail("该车系不存在");
	}
