...
public class AutoSeriesPipeline implements Pipeline {
	...
	public void process(ResultItems resultItems, Task task) {
		AutoBrand brand = (AutoBrand) resultItems.get("autoBrand");
		List<AutoSeries> seriesList = (List<AutoSeries>) resultItems.get("seriesList");
		logger.info("开始保存汽车之家车系数据，品牌ID: {}, 品牌名称: {}, 车系长度: {}", brand.getAutoHomeId(), brand.getName(), seriesList.size());
		for (AutoSeries series : seriesList) {
			logger.info("保存汽车之家车系数据，车系ID: {}, 车系名称: {},", series.getAutoHomeId(), series.getName());
			autoSeriesService.saveByCrawl(series);
		}
