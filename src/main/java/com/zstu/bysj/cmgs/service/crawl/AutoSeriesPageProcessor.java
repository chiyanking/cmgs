...
public class AutoSeriesPageProcessor implements PageProcessor {
	...
	public void process(Page page) {
		...
		this.autoBrand = autoBrandService.findByAutoHomeId(brandId);
		// 解析车系列表
		...
		// 数据输出
		page.putField("autoBrand", this.autoBrand);
		page.putField("seriesList", seriesList);
		for (AutoSeries series : seriesList) {
			logger.info("解析车系： {}[{}], {}, 价格 {}-{}万", series.getName(), series.getAutoHomeId(), series.getClassify(),
			...
			private List<AutoSeries> parseAutoSeries(Page page) {
				...
				for (Selectable seriesNode : seriesNodes) {
					...
					String classify = seriesNode.xpath("//span[@class='info-gray']/text()").get();

			AutoSeries autoSeries = new AutoSeries();
			// 如果品牌和车系名称相同，或者品牌名称为英文或数字的，不做前缀去除处理
			if (StringUtils.equals(seriesName, autoBrand.getName()) || Pattern.matches("[\\w|\\d]*", autoBrand.getName())) {
				autoSeries.setName(seriesName);
			} else {
				// 去除前缀与品牌一致的字符
				autoSeries.setName(StringUtils.removeStart(seriesName, autoBrand.getName()).trim());
			}
			autoSeries.setAutoHomeId(seriesId);
			autoSeries.setBrandId(autoBrand.getId());
			autoSeries.setMark(PinYinTransfer.single().toPinyin(autoBrand.getName()));
	...
	private void parsePageSplits(Page page) {
		...
		List<Selectable> aNodes = page.getHtml().xpath("//div[@class='price-page']/div[@class='page']")
		                              .css("a[href~=/price/autoBrand-]").nodes();
		if (aNodes.size() > 0) {
